...
.`.env` , находятся ссылки на данные, которые скрываем (эти данные получаем через dotenv)
.`Allure`, для генерации отчетов
.`Docker`- инструмент, чтобы запускать всю сборку локально, без сохранения установленных окружений на личном компьютере.
.В проекте используется фреймворк `Pytest`, `Selenium`, для UI тестов. Тесты запускаются через `Pytest`. Тесты могут запускаться удаленно, через `Selenoid`,в котором используется контейнеры `Docker` для запуска определенных версий браузеров, так и локально (на вашем компьютере) можно запустить (если у вас установлены драйвера для браузеров).
## :spiral_notepad: Реализованы проверки

### &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; UI

>- [x] *Проверка страницы авторизации*
>- [x] *Проверка title страницы фильтра каталога*
>- [x] *Проверка title главной страницы*
>- [x] *Проверка страницы поиска(релевантность поиска)*
>- [x] *Проверка наличия кнопки перехода на главную страницу, если в каталоге нету товаров*
>- [x] *Проверка заголовков страницы регистрации*
...
>- [x] *Добавление товара через карточку товара(вместе с негативными кейсами)*
>- [x] *Авторизация через админку (негативные)*
>- [x] *Добавление товара через админку*


## Подготовка окружения
1. Нужно создать файл .env внутри скаченного репозитория test_repo. Путь к этому файлу будет выглядеть так test_repo/.env
2. В .env файл записать
```
PASSWORD='bitnami'
USER='user'
```
...
1. Скачиваем этот репозиторий и переходим через 'CMD' в test_repo, эта папка в скаченного репозитория.
...
```
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;:green_circle:&nbsp;&nbsp;*Запуск тестов в браузере без интерфейса(быстрее тесты проходят):*

```bash
pytest --url=http://192.168.0.102:8081 --browser=firefox --remote=False --browser_without_interfaces=true tests
```
...
```bash
pytest --url=http://192.168.0.102:8081 --browser=firefox --remote=True --alluredir=allure-results  tests
```
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;:green_circle:&nbsp;&nbsp;*Сформировать allure отчет после проейденных тестов:*
...
>- [x] *browser_without_interfaces - указатель, запустить ли браузер без интерфейса(при true) или же запускать, чтобы мы видели как проходят тесты на браузере перед нами(при false)*
...
cd docker/opencart
```
5. Открываем в блокноте файл docker-compose.yml и смотрим на 30 строку, там где OPENCART_HOST, сверяемся, чтобы наш ip совпадал с вашим Ip из шага 1.Если не совпадает, то запишите свой Ip, порт не трогайте
6. Запускаем образ
```bash 
docker compose up
```
7. заходим в браузере по `http://ваш_Url_из_шага_1:8081` и вот наш opencard

Если вдруг не зашло, то нужно будет пересобрать образ(удалив valume данные docker).Заходим в файл docker/opencart/docker-compose.yml на строке 30 , там где OPENCART_HOST проставьте Url из шага 1, порт не изменяйте. Повторите шаги выше
...
7. Ищем наш пароль для jenkins, а вот и он 8785767d90e84de89cfd6880aea7a1ba. Находится так, в cmd вводим команду `docker container ls` потом ищем в таблие myjenkins и копируем его id и вводим его в эту команду `docker logs id_myjenkins_который_увидели` и тут в консоли увидим строчку одну, которая будет выглядеть как этот код 8785767d90e84de89cfd6880aea7a1ba ( у вас может быть другим, это нормально)
...
11. И сейчас подготовим данные, для команды импорты job-ы в jenkins

    11.1 8785767d90e84de89cfd6880aea7a1ba этот пароль берется из шага 7
    ...
    11.2 Данные готовы и вот команда
```bash 
...
```
12. нажимаем `Настроть Jenkins`->`Управление плагинами`->`Available plugins`-> в input вводим `allure` ->в списке появится наш allure(выбираем его)->ставим галочку рядом с нашим allure и устанавливаем с перезагрузкой
...
### :pushpin: Параметры сборки Jenkins
...
*После выполнения сборки(прохождения тестов на удаленном сервере), в (Jenikns) блоке <code><strong>*История сборок*</strong></code>, напротив номера сборки, появится
...
cd docker/selenoid
```
5. Создаем образ Selenoid
```bash 
docker compose up
```
6. Подгружаем браузеры chrome( можно и firefox, opera и другие версии указать)
```bash 
...
```
7. заходим в браузере по `http://ваш_Url_из_шага_1:8082` и вот наш selenoid
<p align="center">
  <img src="images/selenoid_chrome.jpg" alt="video" width="1000">
</p>
Эта картинка с запущенным тестом, не пугайтесь, если у вас не так, тут тесты запустил

## Запуск тестов внтури dockerfile
1. Сейчас мы внутри папки test_repo `%USERPROFILE%\Documents\hd\4\test_repo`
2. Запускаем программу `Docker` (если запущена, то пропускаем)
3. копируем файл, в cmd вводим команду. Так мы Dockerfile переместим выше test_repo и файл будет на одном уровне с папкой test_repo
```bash 
copy test_repo\docker\pytest\Dockerfile ..\
```
4. Создаем образ pytest с нашими тестами
```bash 
cd ..
```
```bash 
